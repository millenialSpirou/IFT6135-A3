r"""
:mod:`score` -- Pytorch code for evaluating generative image model
================================================================================

.. module:: score
   :platform: Unix
   :synopsis: Code is ad{a/o}pted from https://github.com/ajbrock/BigGAN-PyTorch.
...
class WrapInception(nn.Module):
    ...
    def forward(self, x):
        # Normalize x  (XXX: expects `x \in [-1, 1]`)
        x = (x + 1.) / 2.0
...
def accumulate_inception_activations(generator, net,
                                     num_inception_images=50000):
    pool, logits = [], []
    while (torch.cat(logits, 0).shape[0] if len(logits) else 0) < num_inception_images:
        with torch.no_grad():
            images = generator().float()
...
def get_scores(generator, num_images=10000, num_splits=10,
               train_filename=None, test_filename=None, net=None):
    if train_filename:
        inception_moms_train = np.load(train_filename)
        data_mu_train = inception_moms_train['mu']
        data_sigma_train = inception_moms_train['sigma']
    if test_filename:
        inception_moms_test = np.load(test_filename)
        data_mu_test = inception_moms_test['mu']
        data_sigma_test = inception_moms_test['sigma']

    if net is None:
        net = load_inception_net(parallel=False)
    pool, logits = accumulate_inception_activations(
        generator, net, num_inception_images=num_images)
    ...
    if train_filename:
        FID_train = numpy_calculate_frechet_distance(
            mu.clone().cpu().numpy(), sigma.clone().cpu().numpy(),
            data_mu_train, data_sigma_train)
    else:
        FID_train = -999
    if test_filename:
        FID_test = numpy_calculate_frechet_distance(
            mu.clone().cpu().numpy(), sigma.clone().cpu().numpy(),
            data_mu_test, data_sigma_test)
    else:
        FID_test = -999
...
def prepare_inception(filename, data_iterator, net=None):
    ...
    batch_size = None
    with torch.no_grad():
        for i, x in enumerate(tqdm(data_iterator)):
            if batch_size is None:
                batch_size = x.size(0)
            pool_val, logits_val = net(x)
            pool.append(pool_val)
            if i + 1 % 1000:
                pool_of_pools.append(torch.cat(pool, 0).cpu())
                del pool
                pool = []
            if (i + 1) * batch_size >= 100000:
                break
    if pool:
        pool_of_pools.append(torch.cat(pool, 0).cpu())
    pool = torch.cat(pool_of_pools, 0).cpu().numpy()
    mu, sigma = np.mean(pool, axis=0), np.cov(pool, rowvar=False)
    np.savez(filename, mu=mu, sigma=sigma, pool=pool)
